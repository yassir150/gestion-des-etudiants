...
public class AffectationAbsenceTableController implements Initializable {
...
//                System.out.println(rs.getInt("etudiant_id"));
    ...
    @FXML
    void ADD_absence(ActionEvent event) {
        Etudiant etudiant = etudiantTable.getSelectionModel().getSelectedItem();
        if (etudiant!= null) {
            try {
                FXMLLoader fxmlLoa = new FXMLLoader(getClass().getResource("MarquerAbsence.fxml"));
                Parent root = (Parent)fxmlLoa.load();
                MarquerAbsenceController controller = fxmlLoa.getController();
                controller.setEtudiant(etudiant);

                Stage stage = new Stage();
                stage.setScene(new Scene(root));
                stage.setTitle("Ajouter abscence");
